...
def on_draw():
    ...
    for row in range(num_rows):
        ...
        for col in range(num_columns):
            ...
            if col < 5 and row == 0:
            ...
            elif col == 5 and row > 0 and (row + MazesIPlayedRated.current_page_value - 1) < len(mazes):
                label = "PLAY"
            elif col == 5 and row == 0:
                label = "NEXT"
            elif col == 5 and row == 16:
                label = "BACK"
            elif col == 0 and 16 > row > 0 and (row + MazesIPlayedRated.current_page_value - 1) < len(mazes):
                maze_creator = mazes[row + MazesIPlayedRated.current_page_value - 1].user
                label = maze_creator
            elif col == 1 and 16 > row > 0 and (row + MazesIPlayedRated.current_page_value - 1) < len(mazes):
                maze_level = mazes[row + MazesIPlayedRated.current_page_value - 1].maze_level
                label = maze_level
            elif col == 2 and 16 > row > 0 and (row + MazesIPlayedRated.current_page_value - 1) < len(mazes):
                maze_rating = mazes[row + MazesIPlayedRated.current_page_value - 1].rating
                formatted_rating = "{:.2f}".format(maze_rating)
                label = formatted_rating
            elif col == 3 and 16 > row > 0 and (row + MazesIPlayedRated.current_page_value - 1) < len(mazes):
                rate_count = mazes[row + MazesIPlayedRated.current_page_value - 1].rate_count
                label = rate_count
            elif col == 4 and 16 > row > 0 and (row + MazesIPlayedRated.current_page_value - 1) < len(mazes):
                ...
                if my_rating is not None:
                    ...
                    label = "None"
            elif col < 5 and 16 > row > 0 and (row + MazesIPlayedRated.current_page_value - 1) >= len(mazes):
                label = ""
